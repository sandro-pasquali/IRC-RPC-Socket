...
function XMLRPC_methodResponse($package)
  {
    // prepare data struct for inclusion
	$params = XMLRPC_prepare($package);
			
	// insert $params into xmlrpc envelope
	$data["methodResponse"]["params"]["param"]["value"] = $params;
            
	// return xmlrpc methodResponse packet
	return(XML_serialize($data,1));  
  }

function XMLRPC_faultResponse($faultCode="666", $faultString="general error")
  {
    // construct fault response data
	$array["methodResponse"]["fault"]["value"]["struct"]["member"] = array();
	$t = &$array["methodResponse"]["fault"]["value"]["struct"]["member"];
	$t[0]["name"] = "faultCode";
	$t[0]["value"]["int"] = $faultCode;
	$t[1]["name"] = "faultString";
	$t[1]["value"]["string"] = $faultString;

	// return xmlrpc faultResponse packet
	return(XML_serialize($array,1));
  }
